...
`REVIEWER_CHANNEL`, `SLACK_SIGNING_SECRET`, `SLACK_BOT_TOKEN`, `SLACK_LEADS_CHANNEL`, `SLACK_REVIEWER_USER_IDS`,
...
| `REVIEWER_CHANNEL` | **The cutover switch (plan §25).** `sms` (default) \| `slack` \| `both`. Chooses where a draft is ANNOUNCED, never who may approve it. An unrecognised value reads as `sms`, so a typo cannot switch off the channel known to work. On `slack`, **the SMS still sends on every Slack failure path** — it just carries a one-segment `/s/` link to the thread instead of the whole draft. Measured saving: **6 segments per lead → 1**. |
| `SLACK_SIGNING_SECRET` | Slack app → Basic Information. Verifies `v0:{timestamp}:{rawBody}` HMAC-SHA256 hex in `x-slack-signature`, 5-minute window checked in BOTH directions. **Fails closed: unset means `/api/slack/*` answer 401 `unconfigured`, and that includes Slack's `url_verification` handshake.** So it must be on the box AND deployed *before* Event Subscriptions is enabled, or the handshake 401s and reads exactly like a broken endpoint. `docs/slack-app-setup.md` §0. |
| `SLACK_BOT_TOKEN` | `xoxb-…`, from OAuth & Permissions after installing to the workspace. Unset = `notifyReviewers` takes the SMS path directly rather than spending a doomed round trip per draft. **Slack answers HTTP 200 with `{"ok":false,"error":…}`, so `lib/slack/client.ts` reads `ok` out of the BODY** — `not_in_channel` means the bot was never `/invite`d. |
| `SLACK_LEADS_CHANNEL` | The `C…` **ID** of `#hh-leads`, not its name — a name produces `channel_not_found`. One channel, party type as a tag. |
| `SLACK_REVIEWER_USER_IDS` | Comma-separated Slack member IDs (`U…`) allowed to approve. **Empty authorises NOBODY** — the same direction as an unset `REVIEWER_PHONES` meaning "text nobody". This list is the ONLY thing that grants `isAdmin` over a Slack identity; a member ID maps to an `admin_users.slack_user_id` row purely to put a NAME on the ledger actor, and an unmapped reviewer is recorded as `SLACK:U…` rather than the anonymous `'ADMIN'` it replaces. **A Slack id can name an admin; it can never create one.** |
