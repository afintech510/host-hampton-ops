...
import { guardRate, guardRecipient, intakeRule } from '@/lib/rateLimit'
import {
 ...
 * number or mail-bomb an address.
 *
 * This was a private `Map<string, number[]>` with its own window constants and
 * no eviction — every string a caller ever submitted stayed in the map for the
 * life of the container, which is an unbounded memory leak keyed by
 * attacker-chosen input, inside a rate limiter. It is now `guardRecipient` in
 * `lib/rateLimit.ts`, sharing that module's swept and capped store (rule 11: a
 * second implementation of "how often may this happen" is one nothing is
 * checking).
 *
 * Note this bounds the RECIPIENT, which is a different question from how often
 * one CALLER may post — `guardRate` below answers that, and neither alone is
 * enough: the recipient bucket stops forty texts to one customer from forty
 * proxies, and the route bucket stops one caller cycling through forty
 * addresses.
 */
...
function isThrottled(identifier: string): boolean {
  return guardRecipient(identifier, {
    label: 'portal/resend-link',
    max: RESEND_MAX_PER_WINDOW,
...
export async function POST(req: NextRequest) {
  // The recipient bucket below stops one address being bombed; this stops one
  // caller walking a list of addresses. 16 real requests in the ten-day nginx
  // window, so `intakeRule`'s 5-per-caller-per-10-minutes is generous here —
  // three is already the per-recipient ceiling.
  const limited = guardRate(req, intakeRule('portal/resend-link'))
