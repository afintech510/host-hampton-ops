...
export default function CMCheerOrderPage() {
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function setupColorToggles() {
      document.querySelectorAll('.color-toggle').forEach((toggle) => {
        toggle.addEventListener('click', (e) => {
          ...
          if (targetImgId && newSrc) {
            const targetImg = document.getElementById(targetImgId) as HTMLImageElement
            if (targetImg) targetImg.src = newSrc
          }
          const siblings = btn.parentElement?.querySelectorAll('.color-toggle')
          siblings?.forEach((sib) => {
            sib.classList.remove('border-cmBlack', 'bg-cmBlack', 'text-white')
    ...
    function calculateTotal(): number {
      ...
      document.querySelectorAll('.qty-input').forEach((input) => {
        ...
        if (el.id === 'qty-patch') {
          total += calculatePatchPrice(qty)
        } else {
          total += qty * parseFloat(el.dataset.price || '0')
        }
      })
    ...
    function alertCustom(msg: string) {
      ...
      toast.className =
        'fixed bottom-10 left-1/2 -translate-x-1/2 bg-cmRed text-white px-8 py-4 rounded-full shadow-2xl z-[9999] font-bold text-sm uppercase tracking-widest'
      toast.innerText = msg
    ...
    const GOOGLE_APPS_SCRIPT_URL =
      'https://script.google.com/macros/s/AKfycbwea47wI6o3Re9rZvXLVHEDKAAFhrE7xp9uNTVDOk0vWGojMKtOmdD0jzr8FVwq-iO3UA/exec'
    ...
    function setupFormSubmit() {
      ...
      orderForm.addEventListener('submit', (e) => {
        ...
        document.querySelectorAll('.qty-input').forEach((input) => {
          ...
          if (qty > 0) {
            ...
            let itemPrice = 0
            if (el.id === 'qty-patch') {
              itemPrice = calculatePatchPrice(qty)
            } else {
              itemPrice = qty * parseFloat(el.dataset.price || '0')
            }
            totalCost += qty * parseFloat(el.dataset.cost || '0')
            orderListHTML += `<li class="flex justify-between border-b border-gray-100 pb-1"><span>${qty}x ${itemName}</span> <span class="text-gray-500">$${itemPrice.toFixed(2)}</span></li>`
            itemsArr.push(`${qty}x ${itemName}`)
          }
        })

        if (!hasItems) {
          alertCustom('Please select at least one item.')
          return
        }
        ...
        const sumItemsList = document.getElementById('summaryItemsList')
        const sumTotal = document.getElementById('summaryTotal')
        const sumConfirmTotal = document.getElementById('summaryConfirmTotal')
        const sumAthlete = document.getElementById('sumAthlete')
        const sumParent = document.getElementById('sumParent')
        const sumPayment = document.getElementById('sumPayment')

        if (sumItemsList) sumItemsList.innerHTML = orderListHTML
        if (sumTotal) sumTotal.textContent = `$${totalAmount.toFixed(2)}`
        if (sumConfirmTotal) sumConfirmTotal.textContent = `$${totalAmount.toFixed(2)}`
        if (sumAthlete)
          sumAthlete.textContent = (document.getElementById('athleteName') as HTMLInputElement)?.value || ''
        if (sumParent)
          sumParent.textContent = (document.getElementById('parentName') as HTMLInputElement)?.value || ''
        if (sumPayment) sumPayment.textContent = paymentMethod

        orderPayload = {
          athleteName: (document.getElementById('athleteName') as HTMLInputElement)?.value || '',
      ...
      finalSubmitBtn?.addEventListener('click', () => {
        if (confirmCheckbox && !confirmCheckbox.checked) {
          alertCustom('Please check the confirmation box to proceed.')
          return
        }
      ...
      closeSuccessBtn?.addEventListener('click', () => {
        ...
        document.querySelectorAll('.color-toggle').forEach((t) => {
          if (t.getAttribute('data-color') === 'black') (t as HTMLElement).click()
        })
      })
    }
  ...
  return (
    <>
      {/* Tailwind Play CDN — scoped to this fundraiser form */}
      <Script src="https://cdn.tailwindcss.com" strategy="beforeInteractive" />
      <Script src="https://unpkg.com/lucide@latest" strategy="afterInteractive" onLoad={() => {
        if ((window as any).lucide) (window as any).lucide.createIcons()
      }} />

      {/* Inline Tailwind config + custom styles for CM Cheer branding */}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        #cm-cheer-root input[type="number"]::-webkit-outer-spin-button {
          -webkit-appearance: none;
          margin: 0;
        }
        #cm-cheer-root input[type="number"] {
      ...
      <div id="cm-cheer-root" className="w-full flex justify-center bg-zinc-100 py-4 sm:py-8 font-sans antialiased text-gray-900">
            ...
            <div className="max-w-4xl mx-auto px-4 py-10 md:py-16 text-center relative z-10">
              <div className="mb-6 relative inline-block group hover:scale-105 transition-transform duration-300">
                ...
                <div className="relative mx-auto w-32 h-32 md:w-48 md:h-48 bg-white rounded-full border-4 border-cmRed shadow-2xl overflow-hidden flex items-center justify-center">
                  <img
                    src="https://images.squarespace-cdn.com/content/66b583cc5e40c13a4c5600b6/beace53a-cee5-46ab-88dd-8db7abc9d773/logo_CM-cheer.png"
                    alt="Center Moriches Cheer Logo"
          ...
          <form id="orderForm" className="bg-white rounded-b-2xl shadow-2xl overflow-hidden border-x border-b border-gray-200">
            ...
            <div className="p-4 md:p-8 border-b border-gray-100 bg-gray-50/50">
              <div className="flex items-center gap-3 mb-8">
                <div className="p-2 bg-cmRed/10 rounded-lg text-cmRed">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full group">
                  ...
                  <div className="w-full aspect-square bg-gray-100 rounded-lg mb-4 flex items-center justify-center overflow-hidden relative">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full group">
                  ...
                  <div className="w-full aspect-square bg-gray-100 rounded-lg mb-4 flex items-center justify-center overflow-hidden relative p-8">
        ...
        <div id="summaryModal" className="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/90 backdrop-blur-md p-4 opacity-0 transition-opacity duration-300">
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden transform scale-90 transition-transform duration-300 flex flex-col max-h-[90vh]">
            <div className="bg-cmBlack text-white p-6 border-b-4 border-cmRed flex justify-between items-center">
              ...
              <button type="button" id="closeSummaryBtn" className="text-gray-400 hover:text-white transition-colors">
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <div className="mb-6">
                ...
                <p className="text-sm"><span className="text-gray-500">Payment Type:</span> <span id="sumPayment" className="font-medium uppercase"></span></p>
              </div>
        ...
        <div id="successModal" className="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/90 backdrop-blur-md p-4 opacity-0 transition-opacity duration-300">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-8 text-center transform scale-90 transition-transform duration-300 border-t-8 border-cmRed">
            ...
            <p className="text-gray-500 mb-8 font-medium">Thank you for supporting CM Cheer! A record of your order has been sent to the team.</p>
            <button id="closeSuccessBtn" className="w-full bg-cmRed hover:bg-red-700 text-white font-bold py-4 rounded-lg transition-colors shadow-lg uppercase tracking-wider">
              Done
            </button>
          </div>
        </div>
